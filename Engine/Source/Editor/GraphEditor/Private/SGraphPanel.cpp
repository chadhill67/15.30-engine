...
void SGraphPanel::Construct( const SGraphPanel::FArguments& InArgs )
{
	...
	AllowConnectionSlicing = InArgs._AllowConnectionSlicing;
...
int32 SGraphPanel::OnPaint( const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled ) const
{
	...
	if (Children.Num() > 0 )
	{
		...
		// Even if we're now in 'slice mode', you need to keep holding the modifiers to actually commit the slice,
		// to basically allow letting go of the modifiers to 'abort' your slice early instead of needing to undo it
		const bool bShouldPreviewSliceLine = CurrentSlice.IsSet() && FSlateApplication::Get().GetModifierKeys().IsAltDown();

		// Tell the connection policy about the slice line before we draw any connections so it will do intersection tests
		if (bShouldPreviewSliceLine)
		{
			FMarqueeRect AbsoluteLine = FMarqueeRect(AllottedGeometry.LocalToAbsolute(GraphCoordToPanelCoord(CurrentSlice->Line.StartPoint)));
			AbsoluteLine.UpdateEndPoint(AllottedGeometry.LocalToAbsolute(GraphCoordToPanelCoord(CurrentSlice->Line.EndPoint)));
			ConnectionDrawingPolicy->SetSliceLine(AbsoluteLine);
		}
		else
		{
		...
		// Pull back data from the drawing policy
		{
			...
			// After drawing all connections, we can draw the slice line on top of them
			if (bShouldPreviewSliceLine && ConnectionDrawingPolicy->DrawSliceLine())
			{
				// Take ownership of the intersected connections to save a copy, since we're about to delete the drawing policy
				const_cast<SGraphPanel*>(this)->CurrentSlice->ConnectionsIntersectingSliceLine = MoveTemp(ConnectionDrawingPolicy->ConnectionsIntersectingSliceLine);
			}
		}
...
FReply SGraphPanel::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if ((MouseEvent.GetEffectingButton() == EKeys::LeftMouseButton) && (MouseEvent.IsAltDown() || MouseEvent.IsControlDown()))
	{
		if (bIsAltDown)
		{
			// If we allow slicing, we want to defer break links to OnMouseButtonUp so we can tell if the mouse was dragged to slice or if we want a normal
			if (AllowConnectionSlicing.Get())
			{
				LastClickSplineOverlap = PreviousFrameSplineOverlap;
				// Don't allow starting a slice while hovering a node, or for read-only graphs
				if (!NodeUnderMousePtr.IsValid() && IsGraphEditable() && !DisplayAsReadOnly.Get())
				{
					const FVector2D GraphMousePos = PanelCoordToGraphCoord( MyGeometry.AbsoluteToLocal( MouseEvent.GetScreenSpacePosition() ) );
					CurrentSlice = FConnectionSlice(GraphMousePos);
					return FReply::Handled().CaptureMouse(SharedThis(this));
				}
			}
			// If we don't allow slicing, run default pin break behavior (executes on mouse down instead of mouse up)
			else
			{
				UEdGraphPin* Pin1;
				UEdGraphPin* Pin2;
				if (PreviousFrameSplineOverlap.GetPins(*this, Pin1, Pin2))
				{
					const UEdGraphSchema* Schema = GraphObj->GetSchema();
...
{
	const bool bIsAltDown = MouseEvent.IsAltDown();
	if (CurrentSlice && MouseEvent.GetEffectingButton() == EKeys::LeftMouseButton && bIsAltDown)
	{
		if (bInSlicingMode)
		{
			ensure(HasMouseCapture());
			const FScopedTransaction SliceTransaction(NSLOCTEXT("GraphEditor", "SliceConnectionsAction", "Slice Connections"));
		
			const UEdGraphSchema* Schema = GraphObj->GetSchema();
		
			for (const TPair<FEdGraphPinReference, FEdGraphPinReference>& ConnectionPair : CurrentSlice->ConnectionsIntersectingSliceLine)
			{
				UEdGraphPin* Pin1 = ConnectionPair.Key.Get();
				UEdGraphPin* Pin2 = ConnectionPair.Value.Get();
		...
		UEdGraphPin* Pin1;
		UEdGraphPin* Pin2;
		if (LastClickSplineOverlap.GetPins(*this, Pin1, Pin2))
		{
			const UEdGraphSchema* Schema = GraphObj->GetSchema();
			Schema->BreakSinglePinLink(Pin1, Pin2);
			CurrentSlice.Reset();
...
FReply SGraphPanel::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	// Keep updating the slice line as long as the mouse button is held, regardless of whether still holding the modifier keys
	// Also consume the mouse move since we don't really want other graph behaviour happening while slicing
	if (CurrentSlice)
	{
		if (this->HasMouseCapture())
		{
			const FVector2D GraphMousePos = PanelCoordToGraphCoord(MyGeometry.AbsoluteToLocal(MouseEvent.GetScreenSpacePosition()));
			CurrentSlice->Line.UpdateEndPoint(GraphMousePos);
			if (!CurrentSlice->Line.GetSize().IsNearlyZero())
			{
				bInSlicingMode = true;
			}
			return FReply::Handled();
		...
		// If we somehow lost mouse capture (eg. window was deactivated) then we should cancel the slice
		CurrentSlice.Reset();
	}
...
void SGraphPanel::MoveNodesToAnchorPinAtGraphPosition(TArrayView<UEdGraphNode*> NodesToMove, FGraphPinHandle PinToAnchor, FVector2D DesiredPinGraphPosition)
{
	...
	else
	{
		...
		TArray<UEdGraphNode*> NodesToMoveCopy(NodesToMove);
...
void SGraphPanel::StraightenConnections()
{
	...
	if ((!bHasAlignedNodes) && CurrentHoveredPins.Num())
	{
		UEdGraphPin* SourcePin = nullptr;
		for (auto& CurrentHoverPin : CurrentHoveredPins)
		{
