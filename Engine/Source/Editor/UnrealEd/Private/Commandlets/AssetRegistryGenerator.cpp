	...
	for (int32 PakchunkIndex = 0; PakchunkIndex < FinalChunkManifests.Num() && bSucceeded; ++PakchunkIndex)
	{
		...
		for (int32 SubChunkIndex = 0; !bFinishedAllFiles; ++SubChunkIndex)
		{
			...
			if (bUseAssetManager)
			{
				...
				// If the Chunk Index is 1000 or greater, we will also check for a unique name via the AssetManager.
				// NOTE: This Index is lowerable, but you run into problems ~Chunk Index 11, so I would keep it high as it's no downside to you.
				if (PakchunkIndex >= 1000) {
				FName RegistryName = UAssetManager::Get().GetUniqueAssetRegistryName(PakchunkIndex);
				if (RegistryName != NAME_None) {
					FString AssetRegistryFilename = FString::Printf(TEXT("%s%sAssetRegistry%s.bin"), *InSandboxFile->GetSandboxDirectory(), *InSandboxFile->GetGameSandboxDirectoryName(), *RegistryName.ToString());
					UE_LOG(LogAssetRegistryGenerator, Display, TEXT("Saved asset registry: %s"), *AssetRegistryFilename);
					ChunkFilenames.Add(AssetRegistryFilename);
				}
}
			}
...
bool FAssetRegistryGenerator::SaveAssetRegistry(const FString& SandboxPath, bool bSerializeDevelopmentAssetRegistry, bool bForceNoFilter)
{
	...
	if (SaveOptions.bSerializeAssetRegistry)
	{
		...
		for (int32 ManifestIndex = 0; ManifestIndex < FinalChunkManifests.Num(); ++ManifestIndex)
		{
			const FChunkPackageSet* Manifest = FinalChunkManifests[ManifestIndex];
			if (Manifest)
			{
				TArray<FName> PackagesInChunk;
				Manifest->GenerateKeyArray(PackagesInChunk);

				for (const FName& PkgName : PackagesInChunk)
				{
					// Capture the returned array directly, not as a pointer
					const TArray<const FAssetData*> AssetsInPackage = State.GetAssetsByPackageName(PkgName);
            
					for (const FAssetData* AssetPtr : AssetsInPackage)
					{
						if (AssetPtr)
						{
							const_cast<FAssetData*>(AssetPtr)->ChunkIDs.AddUnique(ManifestIndex);
						}
					}
				}
			}
