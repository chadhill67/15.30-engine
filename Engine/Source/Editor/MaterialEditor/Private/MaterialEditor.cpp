...
void FMaterialEditor::UpdateMaterialInfoList()
{
	...
	if (TSharedPtr<SGraphEditor> FocusedGraphEd = FocusedGraphEdPtr.Pin())
	{
		...
		{
			MaterialTitleBar->MaterialInfoList->SetVisibility(!CompileErrors.Num() ? EVisibility::Collapsed : EVisibility::Visible);
		}
	}
...
{
	const FGraphPanelSelectionSet SelectedNodes = FocusedGraphEdPtr.Pin()->GetSelectedNodes();
		...
		for (FGraphPanelSelectionSet::TConstIterator NodeIt(SelectedNodes); NodeIt; ++NodeIt)
		{
			...
			if (GraphNode)
			{
				...
				if (Usage && Usage->Declaration)
				{
					...
					{
						FocusedGraphEdPtr.Pin()->SetNodeSelection(DeclarationGraphNode, true);
					}
				}
			}
		}
		FocusedGraphEdPtr.Pin()->ZoomToFit(true);
	}
}
			...
			if (GraphNode)
			{
				...
				for(UMaterialExpression* Expression : Material->Expressions)
				{
					...
					if (Usage && Usage->Declaration == Declaration)
					{
						...
						if (UsageGraphNode)
						{
							FocusedGraphEdPtr.Pin()->SetNodeSelection(UsageGraphNode, true);
						}
					}
				}
			}
	...
	if (SelectedNodes.Num() == 1)
	{
		...
		for (FGraphPanelSelectionSet::TConstIterator NodeIt(SelectedNodes); NodeIt; ++NodeIt)
		{
			...
			if (GraphNode)
			{
				...
				FocusedGraphEdPtr.Pin()->SetNodeSelection(Declaration->GraphNode, true);
			}
		}
	}
}
