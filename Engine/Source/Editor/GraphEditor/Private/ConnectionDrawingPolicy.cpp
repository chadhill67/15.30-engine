...
{
	static bool SegmentIntersection2D(const FVector2D& SegmentAStart, const FVector2D& SegmentAEnd, const FVector2D& SegmentBStart, const FVector2D& SegmentBEnd, FVector2D& OutIntersectionPoint)
	{
		...
		if ( FMath::SegmentIntersection2D(FVector(SegmentAStart.X, SegmentAStart.Y, 0),
			...
			Intersection))
		{
			OutIntersectionPoint = FVector2D(static_cast<float>(Intersection.X), static_cast<float>(Intersection.Y));
			return true;
		}
		return false;
	}
...
#define DrawSpaceLine(Point1, Point2, DebugWireColor) {const FVector2D FakeTangent = (Point2 - Point1).GetSafeNormal(); FSlateDrawElement::MakeDrawSpaceSpline(DrawElementsList, LayerId, Point1, FakeTangent, Point2, FakeTangent, ClippingRect, 1.0f, ESlateDrawEffect::None, DebugWireColor); }
		...
		if (bSliceLineIntersectsSplineBounds)
		{
			...
			{
				const FVector2D Point2 = FMath::CubicInterp(P0, P0Tangent, P1, P1Tangent, TestAlpha + TestStepInterval);
...
bool FConnectionDrawingPolicy::DrawSliceLine()
{
	if (SliceLine.IsValid())
	{
		...
		{
			// We'll draw a dashed line by default, subclasses can override and return false if they don't support calculating slice intersections at all
			const FVector2D LineDelta = SliceLine.EndPoint - SliceLine.StartPoint;
			const FVector2D LineDirection = LineDelta.GetSafeNormal();
			const float LineLength = LineDelta.Size();
			const float DashLength = Settings->SliceLineDashLength;
			FVector2D DashStartPoint = SliceLine.StartPoint;

			TArray<FVector2D> Points;
			Points.Reserve(2);

			// Keep drawing dashes until we hit the end of the line
			float LineLengthRemaining = LineLength;
			while (LineLengthRemaining > 0.f)
			{
				const FVector2D DashDelta = LineDirection * FMath::Min(DashLength, LineLengthRemaining);
				Points.Add(DashStartPoint);
				Points.Add(DashStartPoint + DashDelta);
				...
				// Advance to the next dash, skipping the gap
				// It's fine if we didn't get the full delta since will still subtract the full amount from the length remaining and exit the loop 
				DashStartPoint = Points[1] + DashDelta;
				LineLengthRemaining -= DashLength * 2.f;
				Points.Reset();
			}
		}
		return true;
